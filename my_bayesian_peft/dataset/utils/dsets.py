...
class Mal_CodeDataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
        ...
        dset = load_dataset("Er1111c/Malicious_code_classification")
        super().__init__(dset, tokenizer, 2, self.preamble, add_space, max_seq_len=max_seq_len)
...
Answer:"""
    ...
    def clm_collate_fn(self, batch):
        # No need to use self.add_space here since we add it to the target tokens
        prompts = [self.preamble.format(sentence=e["text"]) for e in batch]
        prompts = self._tokenize_prompts(prompts)
        classes = t.tensor([e["label"] for e in batch])
        targets = t.cat([self.label2target[e["label"]] for e in batch])
        return prompts, classes, targets
    ...
    def s2s_collate_fn(self, batch):
        prompts = [e["text"] for e in batch]
        prompts = self._tokenize_prompts(prompts)
        targets = t.tensor([e["label"] for e in batch])
        return prompts, targets, targets


malicious_code = Mal_CodeDataset
...
class Book_MIA(ClassificationDataset):
    def __init__(
        ...
        print('load file from ', os.path.join('/opt/data/private/LHD_LLM/LLM_uncertainty/bayesian_peft/database', file_path + '.jsonl'))
        dset = datasets.Dataset.from_json(os.path.join('/opt/data/private/LHD_LLM/LLM_uncertainty/bayesian_peft/database', file_path + '.jsonl'))
...
MMLUSplit = {
    ...
    "health": ["anatomy", "clinical_knowledge", "college_medicine", "human_aging", "nutrition", "professional_medicine",
               "virology"],
...
class LMDataset:
    ...
    def _tokenize_prompts_batch(self, prompts):
        prompts = self.tokenizer(prompts, padding=True, return_tensors="pt", truncation=True,
                                 max_length=self.max_seq_len)
        return prompts
...
class TriviaQADataset(LMDataset):
    ...
    def s2s_collate_fn(self, batch):
        ...
        targets_aliases = [self._tokenize_prompts_batch([normalize_answer(a) for a in e["answer"]["aliases"]]) for e in
                           batch]
...
class OpenARCDataset(LMDataset):
    ...
    def _format_samples(self, batch):
        ...
        for e in batch:
            prompts.append(
                self.sample_preamble.format(question=e["question"], answer=normalize_answer(
    ...
    def s2s_collate_fn(self, batch):
        ...
        targets = [self._tokenize_prompts_batch(
            [normalize_answer(self.generate_answer_from_choices(a["choices"], a["answerKey"])) for a in [e]]) for e in
            batch]
...
class DollyDataset(LMDataset):
    def __init__(
        ...
        if subset_name is not None:
            subset_data = dset['train'].filter(lambda example: example['category'] == subset_name).shuffle(
                seed=random_seed)
            other_data = dset['train'].filter(lambda example: example['category'] != subset_name).shuffle(
                seed=random_seed)
