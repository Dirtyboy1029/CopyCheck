...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--bayes-eval-index', type=int, default=1)
    ...
    for active_adapter in self.active_adapters:
        ...
        if self.blobsample:
            ...
            if x.dim() == 2:
                ...
                s_A = torch.ones((x.size(0), self.r[active_adapter]), device=x.device, dtype=x.dtype).uniform_(-1,
                                                                                                               1).sign()
            else:
                r_A = torch.ones((x.size(0), x.size(1), self.in_features), device=x.device, dtype=x.dtype).uniform_(-1,
                                                                                                                    1).sign()
                s_A = torch.ones((x.size(0), x.size(1), self.r[active_adapter]), device=x.device,
                                 dtype=x.dtype).uniform_(-1, 1).sign()

            x = dropout(x)
            lora_noise_a = A_sigma * torch.randn_like(self.lora_A[active_adapter].weight)

            noise = (((x * r_A) @ lora_noise_a.transpose(0, 1)) * s_A) @ self.lora_B[active_adapter].weight.transpose(0,
                                                                                                                      1)
...
class BLoB(WrapperBase):
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, args, accelerator,
        ...
        self.blobconfig = BLoBConfig(bayes_eps=self.args.bayes_eps, bayes_gamma=self.args.bayes_gamma,
    ...
    def _wrap_lora_layer(self, lora_layer):
        ...
        for adapter_name in lora_layer._active_adapter:
            lora_layer.lora_A_rho[adapter_name] = nn.Parameter(
                lora_layer.lora_A[adapter_name].weight.new_zeros(lora_layer.r[adapter_name], lora_layer.in_features))
        ...
        if adapter_name in lora_layer.lora_A.keys():
            if lora_layer.bayes_eps < 0:
                ...
                nn.init.uniform_(lora_layer.lora_A_rho[adapter_name], lora_layer.bayes_eps / math.sqrt(2),
    ...
    def fit_evaluate(self):
        if self.accelerator.is_local_main_process:
            ...
            logging.basicConfig(format='%(asctime)s - %(pathname)s[line:%(lineno)d] - %(levelname)s: %(message)s',
                                level=logging.INFO, filename=save_folder + '/log.txt')
        print(str(self.args.load_model_path))
