    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, args, accelerator,
        ...
        if args.load_lora_path is not None:
            ...
            for i in range(1, self.ensemble_n):
                ...
                self.load_adapter(os.path.join(args.load_lora_path, f'adapter_{i}'), adapter_name=f'adapter_{i}')
    ...
    def fit(self, train_loader, eval_loader):
        def single_train_iter(self, batch, opt, scheduler):
            ...
            if self.args.dataset_type == 'mcdataset':
                ...
                logits = self.base_model(**{'input_ids': batch_in_token_ids, 'attention_mask': attention_mask}).logits
            else:
