...
class WrapperBase(PeftModel):
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, args, accelerator,
        ...
        if args.opt == 'adamw' or args.opt == 'adam' or args.opt == 'adamw_8bit':
    ...
    def fit_evaluate(self):
        if self.accelerator.is_local_main_process:
            save_folder = f'checkpoints/{self.args.model_type}_{self.args.modelwrapper}/{self.args.model}/{self.args.dataset}/{self.args.log_path}'
    ...
    def prepare_for_fit_evaluate(self, dataset, wandb_logger=None):
        ...
        self.base_model, self.opt, train_loader, test_loader, self.scheduler = self.accelerator.prepare(self.base_model,
                                                                                                        self.opt,
                                                                                                        train_loader,
                                                                                                        test_loader,
                                                                                                        self.scheduler)
